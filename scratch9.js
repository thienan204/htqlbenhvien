...
async function main() {
    console.log('Reset is_phu_luc_4_tt25...');
    await prisma.icd10Catalog.updateMany({
        data: { is_phu_luc_4_tt25: false }
    });

    const workbook = xlsx.readFile('mau/phucluc4tt012025.xlsx');
    ...
    // I will not apply any PL1/PL2 business rules to PL4 unless requested.
    
    const finalCount = await prisma.icd10Catalog.count({
        where: { is_phu_luc_4_tt25: true }
    });
    console.log('Total is_phu_luc_4_tt25 = true:', finalCount);
    ...
    const sql = `UPDATE "Icd10Catalog" SET "is_phu_luc_4_tt25" = false;
UPDATE "Icd10Catalog" SET "is_phu_luc_4_tt25" = true WHERE "ma_chi_tiet" IN ('${codesSql.join("', '")}');`;
    
    fs.writeFileSync('update_phuluc4.sql', sql);
    console.log('Exported update_phuluc4.sql successfully.');
}
