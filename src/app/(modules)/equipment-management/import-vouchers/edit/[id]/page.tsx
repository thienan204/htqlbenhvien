...
import { SaveOutlined, ArrowLeftOutlined, PlusOutlined, DeleteOutlined, CheckOutlined } from '@ant-design/icons';
...
export default function EditImportVoucherPage() {
    ...
    const fetchDependenciesAndVoucher = async () => {
        try {
            const [whRes, equipRes, voucherRes, supRes, mfgRes, fundRes] = await Promise.all([
            ...
            if (voucherRes.ok) {
                ...
                if (v.status !== 'PENDING') {
                    message.warning('Phiếu này đã được duyệt, không thể chỉnh sửa!');
    ...
    const onFinishMaster = async (masterValues: any, actionType: 'UPDATE' | 'APPROVE') => {
        try {
            ...
            if (res.ok) {
                message.success(actionType === 'APPROVE' ? 'Đã lưu và duyệt phiếu thành công!' : 'Đã cập nhật phiếu nháp!');
                router.push('/equipment-management/import-vouchers');
            } else {
        ...
        <div className="p-6 bg-slate-50 min-h-screen">
            <div className="flex justify-between items-center mb-6">
                <Space>
                    ...
                    <Title level={3} className="!mb-0 text-slate-800">Sửa Phiếu Nhập Kho</Title>
                </Space>
                <Space>
                    <Button type="default" icon={<SaveOutlined />} onClick={() => { setSubmitAction('UPDATE'); masterForm.submit(); }} loading={loading} size="large" className="border-blue-600 text-blue-600 font-medium">
                        Cập nhật Phiếu (Lưu Nháp)
                    </Button>
                    <Button type="primary" icon={<CheckOutlined />} onClick={() => { setSubmitAction('APPROVE'); masterForm.submit(); }} loading={loading} size="large" className="bg-green-600 font-medium">
                        Cập nhật & Nhập Kho
                    </Button>
                </Space>
            </div>
