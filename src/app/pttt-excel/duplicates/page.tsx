...
import { Table, Button, Spin, Tag, Empty, Input, Modal, Checkbox, Select } from 'antd';
...
export default function DuplicatesPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const db = await openDB('PTTTReaderDB', 3);
    ...
    const maxRuleWidth = filteredDups.length === 0 ? 250 : Math.max(150, Math.min(600, Math.max(...filteredDups.map(item => {
            ...
            render: (violations: any) => {
                ...
                return arr.length > 0 ? (
                    <div className="flex flex-col gap-1">
                        {arr.map((v: string, i: number) => <Tag color="red" key={i} className="whitespace-normal mb-1">{v}</Tag>)}
                    </div>
                ) : null;
            }
        },
        ...
        <div className="flex flex-col h-screen bg-slate-50">
            <div className="p-4 bg-white border-b flex justify-between items-center shadow-sm z-10 shrink-0 gap-4">
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-800 m-0">Danh sách Dữ liệu Trùng lặp PTTT</h1>
