...
function SavedSchedulesContent() {
    ...
    useEffect(() => {
        ...
        setMaKhoa(searchParams.get('maKhoa'));
    }, []);
    ...
    const fetchData = () => {
        if (!date || !maKhoa) {
            return; // Wait for parameters to be loaded
        }
        
        setLoading(true);
        setError('');
        fetch(`/htqlbenhvien/api/clinical-scheduling/get-schedule?date=${date}&maKhoa=${maKhoa}`)
            .then(res => res.json())
            .then(res => {
                if (res.success) {
    ...
    const handleDeleteAll = async () => {
        try {
            setLoading(true);
            const res = await fetch('/htqlbenhvien/api/clinical-scheduling/delete-schedule', {
                method: 'POST',
    ...
    const columns = [
        { title: 'Tên Bệnh nhân', dataIndex: 'ten_bn', key: 'ten_bn', width: 200, render: (text: string) => <strong>{text}</strong> },
    ...
    const filteredData = data.filter(item => {
        const matchName = (item.ten_bn || '').toLowerCase().includes(searchName.toLowerCase());
