...
export default function DetailedReport() {
    ...
    const handleSaveErrorsToDB = async () => {
        const errorRows = filteredDataSource.filter(r => r.isError);
        ...
        const errorsToSave = errorRows.map(item => {
            const parseDateString = (d: string) => {
                if (!d) return null;
                // format in table is DD/MM/YYYY or DD/MM/YYYY HH:mm
                const parts = d.split(' ');
                const dateParts = parts[0].split('/');
                if (dateParts.length === 3) {
                    const timeStr = parts[1] ? `T${parts[1]}:00` : 'T00:00:00';
            ...
            return {
                ...
                ho_ten: item.ho_ten || '',
                ma_the: item.ma_the || '',
                ngay_vao: parseDateString(item.ngay_vao),
                ngay_ra: parseDateString(item.ngay_ra),
                ngay_yl: parseDateString(item.ngay_yl),
                ngay_th_yl: parseDateString(item.ngay_th_yl),
                ngay_kq: parseDateString(item.ngay_kq),
                ngay_vao_noi_tru: parseDateString(item.ngay_vao_noi_tru),
                ma_dv: item.ma_dv || '',
                ten_dv: item.ten_dv || '',
